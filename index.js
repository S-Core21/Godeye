            ...
            if (Type === "SWAP") {
             ...
             await transferMessage(webhookEvent, wallet, wallet2, sig, Source, solcAcct, desc, sol, AW1, bot, user, address1, address2, buyButtons)
              ...
              if(wallet){
                ...
                const messageToSend = `${walletgroup(wallet.group)} ALERT \n👤*${wallet.name}* *BURNED 🔥* *${formatMcap(desc[2])} ${dexresult.ticker}*(${await soldollarvalue(mint, tokenName)})\n\n*💡${dexresult.ticker} | MC: ${dexresult.mcap}*\n\`${mint}\`\n🔎 DYOR: [SOLC](${txid}) | [X](${dexresult.twitter}) | [RICK](${dexresult.rick})| [DS](${dexresult.Dexscreener}) | [DT](${dexresult.Dextools}) | [BE](${dexresult.Birdeye}) | [Pump](${dexresult.pump})\n\n🕵️‍♂️ *Analyse Wallet:* [W1](${AW1}${UserAccount})\n\`${UserAccount}\` ➡️ [${wallet.name}](${solcAcct}${wallet.address})`
